...
def read_as_delimited(logger: loggers.Logger, fp: Path) -> pd.DataFrame:
    log = logger.spawn(fp.name)
    with log.timer('converted xlsx to tsv to DataFrame'), io.StringIO() as buffer:
        xlsx2csv.Xlsx2csv(fp, delimiter='\t').convert(buffer)
        buffer.seek(0)
        ti = time.perf_counter()
        log.debug(f'Loading `{fp.name}`...')
        wb = pd.read_csv(buffer, sep='\t')
        log.info(f'Loaded {fp.name} ({format_time(time.perf_counter() - ti)})')
        return wb
...
class Parser:
    ...
    def run(self, dat_fp: Path, agile_fp: Path, usda_url: str, connection_string: str) -> None:
        ...
        with ThreadPoolExecutor(max_workers=4) as self.threads:
            ...
            agile_future = self.threads.submit(read_workbook_buffer, self.log, agile_fp,
                                               ['Sheet1'])
            db_future = self.threads.submit(self.consumer, connection_string)
    ...
    def consumer(self, connection_string: str) -> None:
        ...
        log.info(f'Built {connection_string} ({format_time(time.perf_counter() - ti)})')
